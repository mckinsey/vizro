...
- Component height = rows × row_min_height
...
Choose by data type—don’t default to Dropdown for everything:
...
Page-level filters without `targets:` apply to **all** components. If any component uses a pre-aggregated dataset without the filter column, Vizro raises "column not found". **Always specify `targets:`** when using pre-aggregated data, listing only components whose datasets contain that column.
...
3. **Layouts**: Grid or Flex only (no absolute positioning)
4. **Components**: Graph, AgGrid, Card, Figure
