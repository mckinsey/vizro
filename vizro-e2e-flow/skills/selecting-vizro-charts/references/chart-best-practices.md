...
Plotly Express does **not** aggregate. Bar and line charts on detail-level data stack individual rows as separate rectangles instead of summing — producing visually broken charts. Use `@capture("graph")` functions that aggregate inside for bar and line charts.
