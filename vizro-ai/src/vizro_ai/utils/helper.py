...
GENERAL_CHART_BEST_PRACTICES = {
    "color_usage": "Avoid using plotly color palettes/color scales unless it's suggested in visual vocabulary or the user explicitly requests them.",
...
def _get_augment_info(chart_type: str, chart_code: str, user_input: str) -> str:
    ...
    return f"""
    You are tasked with improving the chart code while maintaining the same chart type and basic functionality.
    ...
    3. Carefully study and implement the exact approach used in the best practice code below. Focus on learning the implementation logic:
       - Study the best practice code below to understand its advantages over the current code
       - For complex charts (e.g., lollipop/dumbbell): analyze how the figure is constructed step by step
       - For simple charts: maintain the straightforward approach if it's already optimal
