...
from vizro_ai.dashboard._pydantic_output import _create_message_content, _get_pydantic_model


def test_get_pydantic_model_valid(component_description, expected_card):
    result = _get_pydantic_model(query=component_description, llm_model=TestModel, response_model=vm.Card, df_info=None)

    assert result.model_dump(exclude={"id": True}) == expected_card.model_dump(exclude={"id": True})


def test_get_pydantic_model_invalid(component_description):
    with pytest.raises(ValueError, match="1 validation error for Card"):
        _get_pydantic_model(query=component_description, llm_model=TestModel, response_model=vm.Card, df_info=None)
