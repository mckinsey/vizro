...
In case those issues concern code execution, note that `vizro_ai` does not execute model-generated code (see [Code execution and data handling in Vizro-AI](safeguard.md)).

## Code execution in Vizro-AI

Vizro-AI does not execute code generated by the model. Chart creation produces a declarative specification that Vizro-AI renders with its own trusted `plotly.express` code. You should still:

- Review the generated chart specification (and the `code` string, if you use it) before relying on it in sensitive contexts.
- Provide the dataframe from a data source you trust, already in the shape you want to plot — Vizro-AI does not transform data.
- Review and understand the selected model before connecting it with `vizro_ai`.

To learn more, see [Code execution and data handling in Vizro-AI](safeguard.md).

## Debugging and observability in Vizro-AI

Vizro-AI uses [Pydantic AI](https://ai.pydantic.dev/) which supports integration with observability tools. You can use tools like [Logfire](https://pydantic.dev/logfire) for debugging and monitoring your agent runs.

For example, to enable Logfire observability:
