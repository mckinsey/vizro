...
page_gapminder_2007 = vm.Page(
    ...
    components=[
        vm.Graph(
            id="gapminder_scatter_implicit_cascade",
            title="Implicit default Cascader (multi=True)",
            figure=px.scatter(GAPMINDER_2007, **_GAP_SCATTER),
        ),
        vm.Graph(
            id="gapminder_scatter_explicit_cascade",
            title="Explicit vm.Cascader(multi=False)",
            figure=px.scatter(GAPMINDER_2007, **_GAP_SCATTER),
        ),
    ],
    controls=[
        vm.Filter(
            targets=["gapminder_scatter_implicit_cascade"],
            column=["continent", "country"],
        ),
        vm.Filter(
            targets=["gapminder_scatter_explicit_cascade"],
            column=["continent", "country"],
            selector=vm.Cascader(multi=False, title="Continent → country (single leaf)"),
        ),
    ],
)
