...
secrets_data = pd.DataFrame(
    {
        ...
        "Severity": ["Critical", "High", "High", "Critical", "Medium"],
    }
)
...
composition_data = pd.DataFrame({"Severity": ["Critical", "High", "Medium"], "Count": [25, 75, 150]})
...
def create_bar_chart(data_frame, x, y, color_seq=None):
    return px.bar(data_frame, x=x, y=y, color_discrete_sequence=color_seq or ["#1f77b4"], height=400)
    ...
    components=[
        ...
        vm.Container(
            ...
            components=[
                ...
                vm.AgGrid(title="Top Issues (289 rules)", figure=dash_ag_grid(top_issues)),
            ],
        ),
...
# Second page with empty card
page2 = vm.Page(
    title="Reports",
    controls=[
        vm.Filter(column="Severity"),
    ],
    layout=vm.Grid(grid=[[0, 0], [1, 2], [1, 2]]),
    components=[
        vm.Card(
            text="""
            ## Security Reports

            This section is under construction. Future reports will include:
