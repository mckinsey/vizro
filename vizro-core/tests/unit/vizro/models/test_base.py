...
# @pytest.fixture
# def fresh_models():
#     """Fixture that provides clean model state for each test to avoid interference."""
#     # Get all model classes from vm namespace that might be modified by add_type
#     all_models = [getattr(vm, name) for name in vm.__all__ if hasattr(getattr(vm, name), "model_fields")]

#     # Store original field definitions for all models
#     original_fields = {}
#     for model in all_models:
#         if hasattr(model, "model_fields"):
#             original_fields[model] = model.model_fields.copy()

#     yield

#     # Restore all models to their original field definitions
#     for model, original_field_dict in original_fields.items():
#         model.model_fields.clear()
#         model.model_fields.update(original_field_dict)
