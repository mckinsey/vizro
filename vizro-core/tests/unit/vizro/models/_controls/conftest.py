...
@pytest.fixture
def managers_one_page_two_graphs(gapminder_with_datetime):
    """Instantiates a simple model_manager and data_manager with a page, and two graph models and gapminder data."""
    vm.Page(
        id="test_page",
        title="My first dashboard",
        components=[
            vm.Graph(id="scatter_chart", figure=px.scatter(gapminder_with_datetime, x="lifeExp", y="gdpPercap")),
            vm.Graph(id="bar_chart", figure=px.bar(gapminder_with_datetime, x="country", y="gdpPercap")),
        ],
    )
    Vizro._pre_build()
