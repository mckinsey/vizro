...
class TestKPICardReference:
    ...
    def test_reference_format_missing_placeholder(self):
        with pytest.raises(
            ...
            kpi_card_reference(data_frame=df, value_column="Actual", reference="Reference", reference_format="{.2f}}")()

    def test_reference_format_non_existing_placeholder(self):
        with pytest.raises(KeyError, match="delta_absolute"):
            kpi_card_reference(
                data_frame=df, value_column="Actual", reference="Reference", reference_format="{delta_absolute.2f}}"
            )()
