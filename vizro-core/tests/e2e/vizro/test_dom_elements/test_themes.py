...
def test_themes(dash_br, dashboard_id):
    ...
    if dashboard_id == cnst.DASHBOARD_DEFAULT:
        check_graph_color(dash_br, style_background=cnst.STYLE_TRANSPARENT, color=cnst.HEX_GREY)
        check_theme_color(dash_br, color=cnst.THEME_LIGHT)
        dash_br.multiple_click(theme_toggle_path(), 1)
        check_graph_color(dash_br, style_background=cnst.STYLE_TRANSPARENT, color=cnst.HEX_BLACK)
...
def test_themes_page_change(dash_br, dashboard_id):
    ...
    if dashboard_id == cnst.DASHBOARD_DEFAULT:
        _logic(style_background=cnst.STYLE_TRANSPARENT, graph_color=cnst.HEX_BLACK, theme_color=cnst.THEME_DARK)
    else:
        _logic(style_background=cnst.STYLE_TRANSPARENT, graph_color=cnst.HEX_GREY, theme_color=cnst.THEME_LIGHT)
