...
class export_data(CapturedActionCallable):
    ...
    def function(
        ...
        targets: Optional[list[ModelID]] = None,  # TODO FUTURE: maybe rename this so it doesn't inconsistently use
        # targets?
    ...
    def outputs(self) -> dict[str, Output]:
        ...
        targets = targets or [
            ...
            for model in cast(
                ...
                model_manager._get_models(FIGURE_MODELS, model_manager._get_model_page(self._action)),
            )
        ]

        return {
            f"download_dataframe_{target}": Output(
                component_id={"type": "download_dataframe", "action_id": self._action.id, "target_id": target},
    ...
    def components(self) -> list[dcc.Download]:
        ...
        return [
            dcc.Download(id={"type": "download_dataframe", "action_id": self._action.id, "target_id": target})
            for target in targets
        ]
