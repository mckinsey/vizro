...
* `hatch run test-integration` runs integration tests that includes checking that the example apps in `vizro-core/examples` run.
...
`hatch run pip` provides a [pip-compatible interface using uv](https://docs.astral.sh/uv/pip/). You should not need to use this much.
