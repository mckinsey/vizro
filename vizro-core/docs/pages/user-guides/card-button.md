...
!!! example "Card" === "app.py" \`\`\`{.python pycafe-link} import vizro.models as vm from vizro import Vizro
...
!!! example "Card with custom text" === "app.py" \`\`\`{.python pycafe-link} import vizro.models as vm from vizro import Vizro
...
!!! example "Card with image" === "app.py" \`\`\`py import vizro.models as vm from vizro import Vizro
...
!!! example "Card with styled image" === "images.css" `css     img[src*="#my-image"] {       width: 120px;       height: 120px;     }     ` === "app.py" \`\`\`py import vizro.models as vm from vizro import Vizro
...
!!! example "Card with floating image" === "images.css" `css     img[src*="#my-image"] {       width: 120px;       height: 120px;     }     ` === "app.py" \`\`\`py import vizro.models as vm from vizro import Vizro
...
!!! example "Card with icon" === "app.py" \`\`\`{.python pycafe-link} import vizro.models as vm from vizro import Vizro
...
!!! example "Responsive icon" ![responsive icon](../../assets/user_guides/components/responsive_icon.gif)
...
!!! example "Navigation Card" === "app.py" \`\`\`{.python pycafe-link} import vizro.models as vm import vizro.plotly.express as px from vizro import Vizro
...
To enhance dashboard interactions, you can use the \[`Button`\]\[vizro.models.Button\] component to trigger any pre-defined action functions such as exporting chart data. To use the currently available options for the \[`Actions`\]\[vizro.models.Action\] component, check out the \[API reference\]\[vizro.actions\].

To add a \[`Button`\]\[vizro.models.Button\], insert it into the `components` argument of the \[`Page`\]\[vizro.models.Page\].

You can configure the `text` argument to alter the display text of the \[`Button`\]\[vizro.models.Button\] and the `actions` argument to define which action function should be executed on button click.

In the below example we show how to configure a button to export the filtered data of a target chart using \[export_data\]\[vizro.actions.export_data\], a pre-defined action function.
