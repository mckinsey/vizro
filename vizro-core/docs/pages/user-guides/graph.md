...
In the Python example we directly inserted the pandas DataFrame `df` into `figure=px.scatter_matrix(df, ...)`. This is [the most straightforward way to connect a chart to data](data.md#supply-directly). For the YAML version, we [refer to the data source by name as `data_frame: iris`](data.md#reference-by-name). For a full explanation of the different methods you can use to provide data to your dashboard, see [our guide to using data in Vizro](data.md).
