...
The examples on this page use the iris dataset, which can be generated by calling `px.data.iris()`. You can alternatively download the iris data from [Vizro's GitHub repository](https://raw.githubusercontent.com/mckinsey/vizro/main/vizro-core/docs/assets/user_guides/data/iris.csv), then store it and read it from file.
...
!!! example "Static data referenced by name"
    === "app.py"
        ```{.python pycafe-link extra-requirements="pyYAML"}
        import yaml
        ...
        data_manager["iris"] = px.data.iris() # (1)!
        ...
        1. `"iris"` is the name of a data source added to the data manager. This data is a pandas DataFrame created from the CSV file `iris.csv`.
    === "dashboard.yaml"
        ```yaml
        pages:
        - components:
...
!!! example "Dynamic data"
    === "app.py"
        ...
        1. `iris` is a pandas DataFrame created from `px.data.iris()`
...
!!! example "Parametrized dynamic data"
    === "app.py"
        ...
        2. `iris` is a pandas DataFrame created from `px.data.iris()`.
