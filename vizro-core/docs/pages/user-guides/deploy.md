...
When developing your dashboard you typically run it _locally_ (on your own computer) using the Flask development server. When you deploy to production, this is no longer suitable. Instead, you need a solution that is set up to handle multiple users in a stable, secure and efficient fashion.

Vizro benefits from [Dash's stateless architecture](https://dash.plotly.com/sharing-data-between-callbacks#dash-is-stateless), which makes it simple to scale your app to many concurrent users. Vizro is designed to be a production-ready framework, which means that the dashboard you created during development is immediately suitable for deployment to production with minimal changes.

In general, there is only one code change that is required. The last of your `app.py` file probably looks like `Vizro().build(dashboard).run()`. This should be replaced with:
...
[Hugging Face](https://huggingface.co/) is a platform for machine learning models, datasets and demos. [Hugging Face Spaces](https://huggingface.co/spaces/launch) offers a one click experience to deploy a Vizro dashboard for free. This is the easiest way to deploy a Vizro app if you do not mind your app's code being public or shared within your [Hugging Face organization](https://huggingface.co/organizations). Paid plans include features such as authentication, network security, user analytics and more powerful computing resources.

The best way to get started with Vizro on Hugging Face is to copy an existing Vizro dashboard and then modify it to add your own app. Vizro maintains an [official gallery of example dashboards](https://huggingface.co/collections/vizro/vizro-official-gallery-66697d414646eeac61eae6de) and a [gallery of example dashboards made by members of the community](https://huggingface.co/collections/vizro/community-demos-666987c8e9f56afc7bc1d0fc). Any of these example dashboards can be used as a template for your app. In Hugging Face terminology, a Vizro dashboard lives in a _Space_, and you can copy a dashboard by [duplicating the Space](https://huggingface.co/docs/hub/en/spaces-overview#duplicating-a-space).

If this is your first Vizro deployment then we recommend using our ["first dashboard" example](https://huggingface.co/spaces/vizro/demo-first-dashboard). This is a minimal example that is designed to make it as simple as possible to get started. To create your own Vizro deployment on Hugging Face based on this template, follow the below steps.
...
3. 
4. https://huggingface.co/docs/hub/en/spaces-overview#duplicating-a-space
3. 
4. (). 
...
Sign up Click  Screenshot of what you get

Explain what files are link to dockerfile explanation below

More things you can do on HF:

- can edit metadata
- look at our gallery to see more advanced demos
- click "like" for dashboards, modify them with git

Refer to general princpiles also
...
Although these services work in slightly different ways, there are some general principles that usually apply across all hosting providers (including [Hugging Face](#hugging-face) and [Ploomber Cloud](#ploomber-cloud)). In general, the procedure is:
...
1. Specify on your hosting provider how to handle your app:
    - Before the app is started, the requirements should be installed with `pip install -r requirements.txt`.
    - The app should be started with `gunicorn app:app --workers 4`. See the [section on Gunicorn](#gunicorn) for more information.
...
1. Optional: configure further settings on your hosting provider, for example to make your dashboard private or only accessible to certain users.

The method for providing instructions on how to handle your app varies between hosting providers. For example, on Render there are [build and deploy commands](https://render.com/docs/deploys); on Heroku there is a [Procfile](https://devcenter.heroku.com/articles/procfile). One common cross-platform way to configure an environment is using a Dockerfile. This is used by both Hugging Face and Ploomber Cloud. See the [section on Dockerfile](#dockerfile) for more information.
...
- Run [`pip freeze`](https://pip.pypa.io/en/stable/cli/pip_freeze/) in your development environment: `pip freeze > requirements.txt`. On your host server, these dependencies are installed with `pip install -r requirements.txt`.
- Create a new file `requirements.in` for your unpinned requirements and run [`uv pip compile`](https://docs.astral.sh/uv/pip/compile/) in your development environment: `uv pip compile requirements.in -o requirements.txt`. On the host server, these dependencies are installed with `uv pip sync requirements.txt`. This is the method followed in our [KPI dashboard demo](https://huggingface.co/spaces/vizro/demo-kpi/tree/main).

### Gunicorn

[Gunicorn](https://gunicorn.org/) is a production-ready Python server for deploying an app over multiple worker processes. The standard way to write a Vizro app is to have a file called `app.py` contains the Vizro app in a variable called `app`. Given this setup, the command to start Gunicorn with four worker processes is:
