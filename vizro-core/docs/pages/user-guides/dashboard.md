...
Vizro will [automatically display a logo](assets.md/#add-a-logo-image) in the top-left corner of each page if you place an image named `logo.<extension>` in the [assets](assets.md) folder. You can also add [theme-specific logos](assets.md/#theme-specific-logos) named `logo-dark` and `logo-light, `which switch automatically with the active theme. By default, clicking the logo redirects to the homepage.
