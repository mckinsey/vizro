...
!!! note Custom charts can be targeted by [Filters](filters.md) or [Parameters](parameters.md) without any extra configuration. We will showcase both possibilities in the following examples.
...
!!! example "Custom `plotly.express` scatter chart with a `Parameter`" === "app.py"
...
!!! example "Custom `go.Figure()` waterfall chart with a `Parameter`" === "app.py"
