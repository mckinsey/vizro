...
- Time-of-day data (`time` columns containing `datetime.time` objects) uses [`vm.TimePicker(range=True)`][vizro.models.TimePicker]. A column can be converted to this type with [pandas.to_datetime](https://pandas.pydata.org/docs/reference/api/pandas.to_datetime.html) followed by `.time` (for example, `df["t"] = pd.to_datetime(df["t"]).time`).
