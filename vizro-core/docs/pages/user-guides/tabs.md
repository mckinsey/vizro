...
**When to use this:** choose [`Tabs`][vizro.models.Tabs] to let users switch between several [`Container`s][vizro.models.Container] that share the same screen space. To group components into a single visible section, use a [`Container`](container.md) directly; to arrange components on the page as a whole, use [`Page.layout`](layouts.md).
