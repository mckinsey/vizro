...
!!! example "Default built-in Flask development server" === "app.py" \`\`\` from vizro import Vizro import vizro.plotly.express as px import vizro.models as vm
...
The dashboard application can be launched in a Jupyter environment in `inline`, `external`, and `jupyterlab` mode. !!! example "Run in a Jupyter Notebook in inline mode"
...
!!! example "Use Gunicorn" === "app.py" \`\`\`py from vizro import Vizro import vizro.plotly.express as px import vizro.models as vm
