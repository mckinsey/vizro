...
- `url_base_pathname`: serve your Vizro app at a specific path rather than at the domain root. For example, if you host your dashboard at http://www.example.com/my_dashboard/ then you would set `url_base_pathname="/my_dashboard/"`. Alternatively, you can set the environment variable `DASH_URL_BASE_PATHNAME`.
...
- `assets_external_path`: when `serve_locally=False`, you can also set `assets_external_path` to [serve your own assets from a CDN](https://dash.plotly.com/external-resources#load-assets-from-a-folder-hosted-on-a-cdn). Alternatively, you can set the environment variable `DASH_ASSETS_EXTERNAL_PATH`.
